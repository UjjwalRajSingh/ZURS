CLASS lsc_zurs_travel DEFINITION INHERITING FROM cl_abap_behavior_saver.
...
CLASS lsc_zurs_travel IMPLEMENTATION.
    ...
    IF update-travel IS NOT INITIAL.
        ...
        LOOP AT update-travel ASSIGNING FIELD-SYMBOL(<travel_log_update>).
            ...
            IF <travel_log_update>-%control-CustomerId = if_abap_behv=>mk-on.
                ...
                <travel_log_db>-changed_field_name = 'URS_Customer'.
            ...
            IF <travel_log_update>-%control-AgencyId = if_abap_behv=>mk-on.
                ...
                <travel_log_db>-change_id = cl_system_uuid=>create_uuid_x16_static( ).
                <travel_log_db>-changed_field_name = 'URS_Agency'.
...
CLASS lhc_Travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    TYPES:  t_entity_create TYPE TABLE FOR CREATE   zurs_travel,
            t_entity_update TYPE TABLE FOR UPDATE   zurs_travel,
            t_entity_rep    TYPE TABLE FOR REPORTED zurs_travel,
            t_entity_err    TYPE TABLE FOR FAILED   zurs_travel.

    METHODS precheck_reuse_method
        IMPORTING
...
* 11. Behavior Implementation (early numbering)
  METHOD earlynumbering_create.
    DATA: entity        TYPE STRUCTURE FOR CREATE zurs_travel,
...
* 12. Action and Feature Control
  METHOD copyTravel.
    DATA: travels type table for create zurs_travel\\Travel,
          bookings_cba type table for create zurs_travel\\Travel\_Booking,
          booksuppl_cba type table for create zurs_travel\\Booking\_BookingSupplement.
  ...
  METHOD precheck_reuse_method.
    ...
    LOOP AT entities INTO DATA(entity).
        ...
        IF ls_agency-country_code <> ls_customer-country_code.
            ...
            APPEND VALUE #( %cid = COND #( WHEN operation = if_abap_behv=>op-m-create THEN entity-%cid_ref )
                            %is_draft = entity-%is_draft
                            %msg = NEW /dmo/cm_flight_messages( textid = value #( msgid = 'SY'
                                                                                  msgno = 499
                                                                                  attr1 = ' Country codes not matching for Agency & Customer '
                                                                                )
